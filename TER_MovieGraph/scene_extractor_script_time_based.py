...
for i in tqdm(scene_times['scene_id'].tolist()):
    ...
    if scene['valid'].values[0] == 1:
        ...
        saveTimeRangeFrames(probe, movie_file, start_time, end_time, out = f"scene_library/frames/scene_{i}_valid.mp4/", second_skips=second_skips)
        saveTimeRangeVid(movie_file, start_time, end_time, out = f"scene_library/videos/scene_{i}_valid.mp4")
    else:
        start_time = scene['start_time'].values[0]
        end_time = scene['end_time'].values[0]
        saveTimeRangeFrames(probe, movie_file, start_time, end_time, out = f"scene_library/frames/scene_{i}_invalid.mp4/", second_skips=second_skips)
        saveTimeRangeVid(movie_file, start_time, end_time, out = f"scene_library/videos/scene_{i}_invalid.mp4")
